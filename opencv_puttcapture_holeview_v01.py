...
# py opencv_puttcapture_holeview_v01.py --video 200725_PuttHoleView_v01.mov
# py opencv_puttcapture_holeview_v01.py --video 200725_PuttHoleView_v02.mov
...
vidwrite = False #True #toggle to capture video output
...
hole_center = (230, 740) #width height
hole_radius = 60
...
if vidwrite:
    ...
    out = cv2.VideoWriter('PuttOutcomeProcessing_v01.avi', fourcc, 10, (640, 480))
...
def create_black_mask(frame):
    ...
    blackUpper = np.array([255,sensitivity,sensitivity])
...
def screen_contours(cnts, pts_history, xRange, yRange):
    ...
    for cnt in cnts:
        ...
        if cv2.contourArea(cnt) > 4000 or cv2.contourArea(cnt) < 100:
            #cnts.remove(cnt)
            continue
...
#convert putt data to an outcome summary - will need to update this with different conditions for miss/make
def translate_putt_outcome(radius, xcoord, ycoord):
    putt_outcome = 'miss'
    #get coordinates of hole - can make this better with contour mapping
    center_x = hole_center[0]
    center_y = hole_center[1]
    target_radius = hole_radius

    print('xcoord: ' + str(xcoord))
    print('ycoord: ' + str(ycoord))
    print('center_x: ' + str(center_x))
    print('center_y: ' + str(center_y))
    print('target_radius: ' + str(target_radius))

    if (xcoord - center_x)**2 + (ycoord - center_y)**2 < target_radius**2:
        putt_outcome = 'make'
...
def update_puttlog(puttlog, puttid):
    ...
    if cnt_records > 10:
        df = df.iloc[-1,:] #get last record
        last_xcoord = df['xcoord'].min()
        last_ycoord = df['ycoord'].min()
        last_radius = df['radius'].min()
        try:
            putt_outcome = translate_putt_outcome(last_radius, last_xcoord, last_ycoord) #will need this to translate final outcome
            print('outcome: ' + str(putt_outcome))
        except:
            print('Error: translate putt outcome')
        puttlog.append([puttid, int(last_xcoord), int(last_ycoord), int(last_radius), str(putt_outcome)])
...
def write_puttlog(frame, puttlog):
    ...
    if tot_putts > 0:
        while i < tot_putts:
            puttlog_text = 'Putt ' + str(puttlog[tot_putts - i - 1][0]) + ': ' + str(puttlog[tot_putts - i - 1][4]) + ',' + '(' + str(puttlog[tot_putts - i - 1][1]) + ',' + str(puttlog[tot_putts - i - 1][2]) + ')' #add outcome summary
            cv2.putText(frame, puttlog_text, (10, 830 - 30*(i+1)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            i += 1

    return frame

#can use this to calibrate hole location
def get_hole_contour(frame):
    center = None
    radius = 0
    mask_black = create_black_mask(frame) #frame
...
while True:
    ...
    try:
        ...
        cnts = cv2.findContours(mask_backsub.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cnts = imutils.grab_contours(cnts)
        center = None
        ...
        #if no contour seen for a while (bc prev putt is done), update putt log
        else:
    ...
    #cv2.drawContours(frame, [pts_focusArea], -1, (0, 255, 0), 2, cv2.LINE_AA) #use this to draw hole?
    cv2.circle(frame, hole_center, hole_radius, (0, 255, 0), thickness=2, lineType=8, shift=0)
    ...
    if vidwrite:
        ...
        frame_out = cv2.resize(frame[400:840,0:840], (640,480))
        out.write(frame_out)
